...
# #     if not file_path.exists():
# #         raise HTTPException(status_code=404, detail="App script not found")
...
# @dashboard_router.get("/api/state-diff/{sctask}")
# async def get_state_diff(sctask: str):
#     """Get efficient state diff (only changes) for a specific task."""
#     try:
#         from app.services.state_diff_service import StateDiffService
        
#         task_details = dashboard_service.get_task_details(sctask)
#         if not task_details:
#             raise HTTPException(status_code=404, detail=f"Task {sctask} not found")
        
#         before_state = task_details.get("before_state", {})
#         after_state = task_details.get("after_state", {})
        
#         diff_result = StateDiffService.compute_diff(before_state, after_state)
#         summary = StateDiffService.get_summary(diff_result)
...
#         raise
#     except Exception as e:
#         logger.error("Failed to compute state diff for %s: %s", sctask, e)
#         raise HTTPException(status_code=500, detail="Failed to compute state diff")
