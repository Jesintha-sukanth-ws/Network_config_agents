    ...
    def validate_state(
        ...
        for step_index, step in enumerate(workflow, start=1):
            ...
            if intent == "create_vlan":
                ...
                # Validate the target VLAN exists before assigning it to a port.
                if vlan_id is not None and vlan_id not in projected_vlans:
                    step_errors.append({
                        "error_type": "vlan_not_found",
                        "step": step_index,
                        "field": "vlan_id",
                        "message": (
                            f"VLAN {vlan_id} does not exist on the device. "
                            f"Create it before assigning it to a port."
                        ),
                    })
                    execute = False
                    reason = f"VLAN {vlan_id} does not exist"
                ...
                # Validate all allowed VLANs exist before configuring trunk
                missing_vlans = [
                ...
                if missing_vlans:
                    step_errors.append({
                        ...
                        "field": "allowed_vlans",
                        "message": (
                            f"VLANs {missing_vlans} do not exist on the device. "
                            f"Create them before adding to trunk."
                        ),
                    })
                    execute = False
                    reason = f"VLANs {missing_vlans} do not exist"
                elif (
        ...
        hard_error_types = {"duplicate_vlan_name", "vlan_not_found", "interface_routed"}
        has_hard_errors = any(
